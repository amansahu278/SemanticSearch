...
from retrieval.retrievers import build_bm25_retriever, build_deep_retriever, _custom_bm25_preprocessing
...
def get_args():
    ...
    parser.add_argument("--phonetic_enrichment", action="store_true", help="Enable phonetic enrichment for BM25")
    parser.add_argument(
        "--query_expansion", 
        type=str,
...
def initialize_retrievers(chunks, idx_name="faiss_index", top_bm25=5, top_faiss=10, use_phonetic_enrichment=False):
    # BM25 retriever initialization
    bm25_retriever = build_bm25_retriever(chunks, k=top_bm25, use_phonetic_enrichment=use_phonetic_enrichment)
...
def retrieve_and_summarize_phonetic_enrichment(query, bm25_retriever, faiss_retriever, top_keywords=1):
    ...
    combined_docs = sorted(combined_docs, key=lambda x: x.metadata.get('start_time', float('inf')))
    ...
    logger.info(f"Number of documents: {len(bm_25_relevant_docs) + len(faiss_relevant_docs)}")
...
def _query_expansion_preprocessing(chunks, create_phonetic_map=True):
    tokenized_corpus_full = list(map(lambda x: _custom_bm25_preprocessing(x['text']), chunks))
    corpus_vocab = list(set(token for doc_tokens in tokenized_corpus_full for token in doc_tokens))
...
def retrieve_and_summarize_query_expansion(query, bm25_retriever, faiss_retriever, corpus_vocab=None, phonetic_map=None, fuzzy_score_cutoff=95, fuzzy_limit=5):
    logger.info(f"Retrieving with query expansion, mode: {'phonetic' if phonetic_map else 'fuzzy'}")
    # Load tokenizer 
    tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-instruct")
    # tokenizer = tiktoken.encoding_for_model("gpt-4")
    ...
    for term in terms_to_expand:
        ...
        if phonetic_map:
            ...
            # FUZZY
            # --- ---
            fuzzy_matches = fuzz_process.extract(term, corpus_vocab, scorer=fuzz.WRatio, limit=fuzzy_limit)
...
def retrieve_and_summarize_standard(query, bm25_retriever, faiss_retriever, top_keywords=1):
...
if __name__ == "__main__":
    ...
    bm25_retriever, faiss_retriever = initialize_retrievers(chunks, idx_name=filepath.split('/')[-1].split('.')[0], use_phonetic_enrichment=args.phonetic_enrichment)
    ...
    if query:
        if args.phonetic_enrichment:
            retrieve_and_summarize_phonetic_enrichment(query, bm25_retriever, faiss_retriever)
        elif args.query_expansion:
            expansion_type = args.query_expansion
            corpus_vocab, phonetic_map = _query_expansion_preprocessing(chunks, create_phonetic_map= (expansion_type == "phonetic"))
            retrieve_and_summarize_query_expansion(query, bm25_retriever, faiss_retriever, corpus_vocab=corpus_vocab, phonetic_map=phonetic_map)
        else: 
